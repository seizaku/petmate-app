...
export function useCheckVariant<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; productId?: string; name?: string }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
